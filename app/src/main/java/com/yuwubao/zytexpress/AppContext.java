...
import com.yuwubao.zytexpress.utils.SystemUtils;
...
public class AppContext extends Application {
    ...
    public void onCreate() {
        ...
        if (SystemUtils.isInMainProgress(this)) {
            int count = DataBase.getInt(AppConfig.STARTCOUNT);
            //设置默认值
            if (count == 0) {
                firstInitApp();
            }
            count = count + 1;
            DataBase.saveInt(AppConfig.STARTCOUNT, count);
            ImageLoaderKit.create(this);
            DataBase.saveBoolean(AppConfig.ISCHECKVERSION, false);
            //判断设备是否为PDA
            AppConfig.isPDA = isPDA();
        }
